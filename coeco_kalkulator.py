...
<div style='background-color:#1f4e79; padding:16px; border-radius:8px; text-align:center'>
    ...
    <h4 style='color:white;'>Ovitek + jedro | A-formati (A6–A4)</h4>
</div>
""", unsafe_allow_html=True)

st.info("📌 Vnesi število **strani**, ne listov. Ena polo vsebuje dve strani.")
...
with col1:
    format_ovitka = st.selectbox("Format ovitka", list(formati_cm.keys()), index=1)
with col2:
...
sirina_o, visina_o = formati_cm[format_ovitka]
teza_stran_ovitka = (sirina_o * visina_o * gramatura_ovitka) / 10000 / 1000 / 2
teza_ovitka = round(teza_stran_ovitka * strani_ovitka, 6)
...
with col4:
    format_jedra = st.selectbox("Format jedra", list(formati_cm.keys()), index=1, key="format_j")
with col5:
...
sirina_j, visina_j = formati_cm[format_jedra]
teza_stran_jedra = (sirina_j * visina_j * gramatura_jedra) / 10000 / 1000 / 2
teza_jedra = round(teza_stran_jedra * strani_jedra, 6)
...
st.info(f"📘 Skupno: **{skupna_teza} g**")
...
Industrijsko natančen izračun teže natisnjenih strani po formatu, gramaturi in obsegu.
</p>
""", unsafe_allow_html=True)
